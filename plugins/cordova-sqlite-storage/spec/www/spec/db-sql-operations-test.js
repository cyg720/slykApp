    ...
    for (var i=0; i<pluginScenarioCount; ++i) {
      ...
      describe(pluginScenarioList[i] + ': db.executeSql test(s)', function() {
        ...
        it(suiteName + 'db.executeSql store numeric values and check', function(done) {
          ...
          db.executeSql('SELECT entity AS d1, TYPEOF(entity) AS t1, ABS(entity) AS a1, UPPER(entity) as u1 FROM MyTable', [], function (rs) {
            expect(rs.rows).toBeDefined();
        ...
        it(suiteName + 'SELECT UPPER(?) AS upper1, UPPER(?) AS upper2 with "naive" Array subclass (controller NOT explicitly set to subclasss) as value arguments array', function(done) {
        ...
        it(suiteName + 'SELECT UPPER(?) AS upper1, UPPER(?) AS upper2 with "naive" Array subclass (controller explicitly set to subclasss) as value arguments array', function(done) {
          var db = openDatabase('DB-SQL-SELECT-multi-upper-on-array-subclass-explicit-controller.db');
          expect(db).toBeDefined();
